...
## Класс VirtualFileSystem
Назначение: Управление виртуальной файловой системой, основанной на ZIP-архиве.
...
Распаковывает содержимое ZIP-архива во временную директорию. Устанавливает текущую директорию на временную директорию.
...
Реализация команды cd для навигации по файловой системе.
```get_current_dir(self)```

Получение относительного пути текущей директории.Возвращает путь текущей директории относительно корневой директории виртуальной файловой системы.
Для отображения текущей директории в приглашении командной строки.
```read_file(self, filename)```

Чтение содержимого файла. Открывает и читает содержимое указанного файла.
Возвращает список строк файла или сообщение об ошибке, если файл не найден.
...
Класс XMLLogger
Назначение: Логирование действий пользователя в XML-файл.
